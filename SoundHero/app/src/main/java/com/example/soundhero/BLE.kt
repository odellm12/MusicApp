...
class BLE(private val context: Context, deviceName: String):BluetoothGattCallback(), BluetoothAdapter.LeScanCallback {
    ...
    val targetDeviceName = "SoundHero"
...
/*    // Return true if connected to UART device, false otherwise.
    ...
    interface Callback {
        fun onConnected(uart: BLE)
        fun onConnectFailed(uart: BLE)
        fun onDisconnected(uart: BLE)
        fun onReceive(uart: BLE, rx: BluetoothGattCharacteristic)
    ...
    override fun onLeScan(device: BluetoothDevice, rssi: Int, scanRecord: ByteArray) {
        ...
        if (device.name != targetDeviceName) {
            Log.d("BLE", "Target device name  $targetDeviceName")
            return
        }
...
//*************************************************************************//
    ...
    private fun notifyOnConnectFailed(uart: BLE) {
        for (cb in callbacks.keys) {
            cb?.onConnectFailed(uart)
        }
    }

    private fun notifyOnDisconnected(uart: BLE) {
        for (cb in callbacks.keys) {
            cb?.onDisconnected(uart)
        }
    }

    private fun notifyOnReceive(uart: BLE, rx: BluetoothGattCharacteristic) {
        for (cb in callbacks.keys) {
